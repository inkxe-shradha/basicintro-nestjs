...
describe('AuthService', () => {
  ...
  beforeEach(async () => {
    ...
    fakeUserService = {
      ...
      createUser: (email: string, password: string) => {
        const userData = { id: Math.floor(Math.random() * 9999), email, password } as User
        user.push(userData)
  ...
  it('Should able to sign up with a salted and hash password', async () => {
    const user = await service.signup('shradha.praharaj@gmail.com', 'Sh9#923892');
  ...
  it('Should throw an error if the user have invalid email address', async () => {
    await expect(service.signin('asdf@asdf.com', 'abc')).rejects.toThrow(NotFoundException)
  })
